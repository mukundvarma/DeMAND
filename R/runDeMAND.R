...
# To combine p-values of neighboring interactions
.getCombinedPvalue <- function(g, expData, nbsIdxAll, ppiIdx, edgeKLD) {
...
.getKLDpvalue <- function(kld, null) {
...
.getPDM <- function(xdata, ydata, xrange, yrange, sigmaX, sigmaY, covarianceMatrix, globalProbMatrix) {
...
# To get probability density (2-dimensional) from expression data using Gaussian kernel
.getSigKLDedge <- function(yy, bgIndex, fgIndex, expData, globalProbMatrix) {
        ...
        covarianceMatrixBG <- matrix(c(sigmaXbg^2, 0, 0, sigmaYbg^2), nrow = 2, byrow = TRUE)
        ...
        covarianceMatrixFG <- matrix(c(sigmaXfg^2, 0, 0, sigmaYfg^2), nrow = 2, byrow = TRUE)

        # To get probability distribution of case/control using Gaussian kernel method
        sumMvnPmatrixBG <- .getPDM(xdataBG, ydataBG, xrange, yrange, sigmaXbg, sigmaYbg, covarianceMatrixBG, globalProbMatrix)
        sumMvnPmatrixFG <- .getPDM(xdataFG, ydataFG, xrange, yrange, sigmaXfg, sigmaYfg, covarianceMatrixFG, globalProbMatrix)
...
## Main function
runDeMAND <- function (dobj, fgIndex=NULL, bgIndex=NULL){
	...
	flush <- apply(as.matrix(analGene), 1, tempF2 <- function(g) {
		probeIdx <- which(expAnno[, 2] == g)
			...
			tempCV <- apply(tempExpMat, 1, tempF3 <- function(yy) {
				rs <- sd(yy, na.rm = TRUE)/mean(yy, na.rm = TRUE)
                		return(rs)
				})
			tempIdx <<- c(tempIdx, probeIdx[which(tempCV == max(tempCV, na.rm = TRUE))[1]])
        		}
		})
	...
	p1 <- sample(analGene, length(analGene), replace = TRUE)
    	p2 <- sample(analGene, length(analGene), replace = TRUE)
    	pOut <- which(p1 == p2)
    	if (length(pOut) > 0) {p1 <- p1[-pOut]; p2 <- p2[-pOut]}
    	permuteInteractome <- cbind(p1, p2)

    	# To get null distribution of KLD values
	tempNull <- apply(permuteInteractome, 1, .getSigKLDedge, bgIndex, fgIndex, expData, globalProbMatrix)
    	null <- as.numeric(tempNull[3, ])
    
	message("Measure dysregulation of the interactionss.....")

	# To get KLD for all the edges
    	KLDmat <- apply(interactome, 1, .getSigKLDedge, bgIndex, fgIndex, expData, globalProbMatrix)
    	KLDmat <- t(KLDmat)

	# To get pvalue (of KLD) for all the edges
    	KLDpvec <- apply(KLDmat, 1, .getKLDpvalue, null)
	...
	# To combine p-values
    	...
    	intPval <- apply(as.matrix(analGene), 1, .getCombinedPvalue, expData, nbsIdxAll, ppiIdx, edgeKLD)
    	...
    	finalPrediction <- finalPrediction[sort(intPval, decreasing = FALSE, index.return = TRUE)$ix, ]
	row.names(finalPrediction) <- NULL
