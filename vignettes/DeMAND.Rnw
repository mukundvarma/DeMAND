...
\item[anno]{A N-by-2 character matrix where the rows represent probes or genes in the same order as the \code{exp} matrix. The first column must hold the probe id or gene name as appears in the \emph{exp} matrix, and the second column should hold their corresponding names (e.g gene symbol) as appears in the \emph{network} matrix.}
...
\bibitem{Woo2015} Woo, J.H., et al.,  Elucidating Compound Mechanism of Action by Network Dysregulation Analysis in Perturbed Cells. 2015. (under review)
